...
class E2BAgentServiceTestBase(IsolatedAsyncioTestCase):
    """Base class that sets up a FastAPI test client with E2B backend."""

    async def asyncSetUp(self) -> None:
        self.storage = _make_storage()

        self.workspace_manager = E2BWorkspaceManager(
...
@unittest.skipUnless(_E2B_API_KEY, _SKIP_REASON)
class TestE2BAgentCRUD(E2BAgentServiceTestBase):
    """Test agent CRUD endpoints with E2B backend."""

    async def test_create_agent(self) -> None:
        """POST /agent/ creates a new agent and returns its id."""
        resp = await self.client.post(
            "/agent/",
            json={"name": "e2b-agent", "system_prompt": "You help."},
...
@unittest.skipUnless(_E2B_API_KEY, _SKIP_REASON)
class TestE2BSessionCRUD(E2BAgentServiceTestBase):
    """Test session CRUD endpoints with E2B backend."""
...
@unittest.skipUnless(_E2B_API_KEY, _SKIP_REASON)
class TestE2BWorkspaceEndpoints(E2BAgentServiceTestBase):
    """Test workspace MCP and skill endpoints with an E2B backend.

    These tests require a valid E2B_API_KEY because they exercise the full
    workspace lifecycle: sandbox creation, bootstrap, MCP gateway startup,
    and skill upload/listing.
    """
    ...
    async def _setup_session(self) -> tuple[str, str]:
        ...
        agent_resp = await self.client.post(
            "/agent/",
            json={"name": f"e2b-ws-agent-{uuid.uuid4().hex[:6]}"},
            headers=_USER_HEADERS,
        )
    ...
    async def test_list_mcps_empty(self) -> None:
        """GET /workspace/mcp returns empty list for a new E2B workspace."""
    ...
    async def test_add_and_list_skill(self) -> None:
        ...
        try:
            skill_name = "e2b_test_skill"
            skill_md_content = (
                f"---\nname: {skill_name}\n"
                f"description: A test skill for E2B\n---\n\n"
                f"# {skill_name}\n\nTest skill content.\n"
            )
            with open(
    ...
    async def test_remove_skill(self) -> None:
        ...
        skill_dir = tempfile.TemporaryDirectory()
        try:
            skill_name = "removable_e2b_skill"
            with open(
                os.path.join(skill_dir.name, "SKILL.md"),
                "w",
                encoding="utf-8",
...
@unittest.skipUnless(_E2B_API_KEY, _SKIP_REASON)
class TestE2BWorkspaceLifecycle(E2BAgentServiceTestBase):
    """Test workspace lifecycle with E2B backend via the HTTP API.

    Validates that the E2B workspace manager correctly creates, caches,
    and cleans up workspaces through the service API.
    """

    async def test_workspace_created_on_first_access(self) -> None:
        """Accessing workspace endpoints for a session triggers sandbox
        creation and the workspace is cached in the manager."""
        agent_resp = await self.client.post(
            "/agent/",
            json={"name": "e2b-lifecycle-agent"},
            headers=_USER_HEADERS,
        )
        agent_id = agent_resp.json()["agent_id"]
        session_resp = await self.client.post(
        ...
        # Access workspace endpoint — triggers sandbox creation
        resp = await self.client.get(
            "/workspace/mcp",
            params={"agent_id": agent_id, "session_id": session_id},
            headers=_USER_HEADERS,
    ...
    async def test_multiple_sessions_same_workspace_share_cache(self) -> None:
        """Two sessions with the same workspace_id share the same sandbox."""
        agent_resp = await self.client.post(
            "/agent/",
            json={"name": "e2b-shared-agent"},
            headers=_USER_HEADERS,
        )
        agent_id = agent_resp.json()["agent_id"]
...
@unittest.skipUnless(_E2B_API_KEY, _SKIP_REASON)
class TestE2BCrossCutting(E2BAgentServiceTestBase):
    """Cross-cutting tests for the agent service with E2B backend."""
